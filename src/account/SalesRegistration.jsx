...
export default function SalesRegistration() {
  ...
  const getDataShop = async (username, token) => {
    ...
    const response = await callAPI(
      `/api/auth/account/shop/${username}`,
      "GET",
      {},
      config
    );
    ...
    if (
      ...
      if (isConfirmed) {
        ...
        const res = await callAPI(
          `/api/auth/account/saleregis/${accountLogin.username}/${shop_name}?image=${imageUrl}`,
          "POST",
          {
            city,
            district,
    ...
    <React.Fragment>
      <div>
        ...
        <div className="container mt-4">
          {accountLogin &&
          accountLogin.shop &&
          accountLogin.shop.status === 0 ? (
            <div className={`text-danger p-4 text-center`}>
              Bạn đã đăng kí kênh bán hàng, vui lòng chờ phê duyệt!
            </div>
          ) : accountLogin &&
            accountLogin.shop &&
            accountLogin.shop.status === 3 ? (
            <div className={`text-danger p-4 text-center`}>
              Cửa hàng của bạn đã bị cấm hoạt động. Liên hệ Admin để được giải
              quyết!
            </div>
          ) : (
