  ...
  const getdata = async (page, filterStatus) => {
    ...
    try {
      ...
      const response = await callAPI(
        `/api/auth/account/getAll?key=${keyfind}&keyword=${keyword}&offset=${
          page - 1
        }&sizePage=${numberPage}&sort=${sortBy}&sortType=${sortType}&shoporaccount=account`,
        "GET",
        {},
        config
      );
      ...
      if (filterStatus === undefined || filterStatus === "") {
        ...
        const newData = responseData.content.filter(
          (account) => account.status === (filterStatus === "true")
        );
        setAccounts(newData || []);
        setTotalPages(responseData.totalPages || 1);
      }
